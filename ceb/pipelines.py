...
class CebPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `ceb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM ceb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO ceb (date, title, link, content)"
